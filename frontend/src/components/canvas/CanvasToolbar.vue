...
其中第二层动词/宾语生成第一条末层分支，第三层动词/宾语生成第二条末层分支。第二层、第三层的动词都必须像上层一样是可合读成命题的真实关系词，不要使用“进一步”“同时”等篇章词，也不要省略连接词。硬性计数：如果本次共有 N 个带第 5 层的第 4 层节点，则至少 ceil(N/2) 个必须使用完整六段格式，形成两个不同末层分支；不足时请重写，不要用“表现”“结果”“影响”等空泛词凑节点。`
