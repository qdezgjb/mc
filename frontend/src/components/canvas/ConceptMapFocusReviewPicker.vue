...
/**
 * Bottom bar: suggested focus questions only (− / = / 0–3); per-LLM validation is in the AI bar
 */
...
function applySlot(idx: number): void {
  ...
  if (row) store.applyTopicText(row.text)
}
...
function handleKeydown(event: KeyboardEvent): void {
  ...
  if (num >= 1 && num <= 3 && num - 1 < visibleRows.value.length) {
    event.preventDefault()
    event.stopPropagation()
    applySlot(num)
  }
...
<template>
  <div class="concept-focus-review-picker flex flex-col gap-2 min-w-0 w-fit max-w-full">
    <div class="flex flex-col gap-2 min-w-0 w-fit max-w-full">
      <div class="flex items-start justify-between gap-2 min-w-0">
        <p class="text-[10px] text-gray-500 dark:text-gray-400 leading-snug flex-1 min-w-0">
          {{ t('conceptMapPicker.focusAlternativesHint', { pageSize: store.PAGE_SIZE }) }}
        </p>
      ...
      <div class="flex flex-col gap-1.5 w-full min-w-0">
        ...
        >
          ...
          <span
            ...
            >{{ t('conceptMapPicker.currentFocusQuestion') }}</span
          >
        </button>
        <button
