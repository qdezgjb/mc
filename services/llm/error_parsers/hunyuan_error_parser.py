...
def parse_hunyuan_error(
    error_code: str, error_message: str, _status_code: Optional[int] = None
) -> Tuple[Exception, str]:
    """
    ...
    if error_code.startswith("AuthFailure"):
        if "InvalidAuthorization" in error_code:
            if has_chinese:
                ...
                user_msg = "Invalid Authorization header format. Please check authentication configuration."
        ...
        if "InvalidSecretId" in error_code:
            user_msg = (
                "密钥类型错误，请使用云API密钥" if has_chinese else "Invalid secret ID type. Please use cloud API key."
            )
            return LLMAccessDeniedError(
        ...
        if "SecretIdNotFound" in error_code:
            if has_chinese:
                ...
                user_msg = "Secret ID not found or deleted. Please check key configuration."
            return LLMAccessDeniedError(
        ...
        if "SignatureExpire" in error_code:
            if has_chinese:
                ...
                user_msg = "Signature expired. Please check if local time is synchronized with standard time."
            return LLMAccessDeniedError(
        ...
        if "SignatureFailure" in error_code:
            user_msg = (
                "签名错误，请检查签名计算过程"
                if has_chinese
                else "Signature error. Please check signature calculation."
            )
            return LLMAccessDeniedError(
        ...
        if "UnauthorizedOperation" in error_code:
            user_msg = (
                "请求未授权，请检查权限配置" if has_chinese else "Unauthorized operation. Please check permissions."
            )
            return LLMAccessDeniedError(
        ...
        if has_chinese:
            ...
            user_msg = "Authentication failed. Please check API key and signature configuration."
        return LLMAccessDeniedError(
    ...
    # ===== Parameter Errors =====
    if error_code in ("InvalidParameter", "InvalidParameter.InvalidParameter"):
        if has_chinese:
            user_msg = "参数错误，请检查参数格式和类型"
        else:
            user_msg = "Invalid parameter. Please check parameter format and type."
    ...
    if error_code == "InvalidParameterValue" or error_code.startswith("InvalidParameterValue"):
        ...
        if "ParameterValueError" in error_code:
            if has_chinese:
                ...
                user_msg = "Parameter field or value error. Please check parameter configuration."
            return LLMInvalidParameterError(
        ...
        if has_chinese:
            ...
            user_msg = "Invalid parameter value. Please check parameter values."
        return LLMInvalidParameterError(
            f"Invalid parameter value: {error_message}",
            parameter=None,
            error_code=error_code,
    ...
    if error_code == "InvalidRequest":
        if has_chinese:
            ...
            user_msg = "Invalid request format. Please check request body format."
        return LLMInvalidParameterError(
    ...
    if error_code == "MissingParameter":
        if has_chinese:
            ...
            user_msg = "Missing required parameter. Please check parameter configuration."
        return LLMInvalidParameterError(
    ...
    if (
        ...
        or error_code == "RequestLimitExceeded"
        or error_code.startswith("RequestLimitExceeded")
    ):
        user_msg = (
            "请求频率超过限制，请稍后重试" if has_chinese else "Request rate limit exceeded. Please try again later."
        )
    ...
    if error_code.startswith("FailedOperation"):
        if "FreeResourcePackExhausted" in error_code or "ResourcePackExhausted" in error_code:
            if has_chinese:
                ...
                user_msg = "Resource pack exhausted. Please purchase resource pack or enable postpaid."
        ...
        if "EngineServerLimitExceeded" in error_code:
            user_msg = (
                "引擎层请求超过限额，请稍后重试"
                if has_chinese
                else "Engine server limit exceeded. Please try again later."
            )
            return LLMRateLimitError(f"Engine limit exceeded: {error_message}"), user_msg

        if "EngineRequestTimeout" in error_code:
            user_msg = (
                "引擎层请求超时，请稍后重试" if has_chinese else "Engine request timeout. Please try again later."
            )
        ...
        if "ServiceNotActivated" in error_code:
            if has_chinese:
                ...
                user_msg = "Service not activated. Please apply for trial in console."
            return LLMAccessDeniedError(
        ...
        if "UserUnAuthError" in error_code:
            if has_chinese:
                ...
                user_msg = "User not authenticated. Please complete real-name authentication."
            return LLMAccessDeniedError(
    ...
    if error_code.startswith("ResourceUnavailable"):
        ...
        if "StopUsing" in error_code:
            user_msg = (
                "账号已停服，请检查账户状态" if has_chinese else "Account service stopped. Please check account status."
            )
            return LLMAccessDeniedError(
                f"Service stopped: {error_message}",
                provider="hunyuan",
        ...
        if "NotExist" in error_code:
            if has_chinese:
                ...
                user_msg = "Billing status unknown. Please confirm if service is activated in console."
            return LLMAccessDeniedError(
    ...
    if error_code == "ResourceInsufficient.ChargeResourceExhaust":
        if has_chinese:
            ...
            user_msg = "Billing resources exhausted. Please purchase resource pack or recharge."
        return LLMQuotaExhaustedError(
    ...
    if error_code.startswith("OperationDenied"):
        if "TextIllegalDetected" in error_code:
            user_msg = (
                "文本包含违法违规信息，审核不通过" if has_chinese else "Text contains illegal content. Review failed."
            )
        ...
        if "ImageIllegalDetected" in error_code:
            user_msg = (
                "图片可能包含敏感信息，请重试"
                if has_chinese
                else "Image may contain sensitive information. Please retry."
            )
            return LLMContentFilterError(f"Content filter: {error_message}"), user_msg
    ...
    if error_code == "IpNotInWhitelist":
        user_msg = (
            "IP地址不在白名单中，请添加IP到白名单"
            if has_chinese
            else "IP address not in whitelist. Please add IP to whitelist."
        )
        return LLMAccessDeniedError(
    ...
    if error_code == "RequestSizeLimitExceeded":
        if has_chinese:
            ...
            user_msg = "Request size exceeds limit. Please reduce request content."
        return LLMInvalidParameterError(
    ...
    if error_code == "ResponseSizeLimitExceeded":
        if has_chinese:
            ...
            user_msg = "Response size exceeds limit. Please adjust request parameters."
        return LLMInvalidParameterError(
    ...
    if error_code == "ServiceUnavailable":
        user_msg = (
            "服务暂时不可用，请稍后重试" if has_chinese else "Service temporarily unavailable. Please try again later."
        )
    ...
    if error_code == "UnsupportedOperation":
        if has_chinese:
            ...
            user_msg = "Operation not supported. Please check operation type."
        return LLMInvalidParameterError(
    ...
    if error_code == "UnsupportedRegion":
        if has_chinese:
            ...
            user_msg = "Region not supported. Please check region configuration."
        return LLMInvalidParameterError(
