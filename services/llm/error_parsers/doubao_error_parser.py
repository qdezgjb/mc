...
def parse_doubao_error(error_code: str, error_message: str, status_code: Optional[int] = None) -> Tuple[Exception, str]:
    """
    ...
    if status_code == 400 or error_code == "BadRequest":
        ...
        if error_code == "InvalidEndpoint.ClosedEndpoint" or (
            ...
            if has_chinese:
                user_msg = "推理接入点处于已被关闭或暂时不可用，请稍后重试，或联系推理接入点管理员"
            else:
                user_msg = (
        ...
        if "RiskDetection" in error_code or "SensitiveContentDetected" in error_code:
            ...
            if "InputTextRiskDetection" in error_code or (
                "input" in error_code_lower and "text" in error_code_lower and "risk" in error_code_lower
            ):
                if has_chinese:
                    user_msg = "火山引擎风险识别产品检测到输入文本可能包含敏感信息，请您更换后重试"
                else:
                    user_msg = (
                        "Input text may contain sensitive content detected by risk detection. Please modify and retry."
                    )
            ...
            if "InputImageRiskDetection" in error_code or (
                "input" in error_code_lower and "image" in error_code_lower and "risk" in error_code_lower
            ):
                if has_chinese:
                    user_msg = "火山引擎风险识别产品检测到输入图片可能包含敏感信息，请您更换后重试"
                else:
                    user_msg = (
                        "Input image may contain sensitive content detected by risk detection. Please modify and retry."
                    )
            ...
            if "OutputTextRiskDetection" in error_code or (
                "output" in error_code_lower and "text" in error_code_lower and "risk" in error_code_lower
            ):
                if has_chinese:
                    user_msg = "火山引擎风险识别产品检测到输出文本可能包含敏感信息，请您更换后重试"
                else:
                    user_msg = (
                        "Output text may contain sensitive content detected by risk detection. Please modify and retry."
                    )
            ...
            if "OutputImageRiskDetection" in error_code or (
                "output" in error_code_lower and "image" in error_code_lower and "risk" in error_code_lower
            ):
                if has_chinese:
                    user_msg = "火山引擎风险识别产品检测到输出图片可能包含敏感信息，请您更换后重试"
                else:
                    user_msg = (
            ...
            if "SensitiveContentDetected" in error_code:
                if "SevereViolation" in error_code:
                    if has_chinese:
                        ...
                        user_msg = "Input text may contain severe violation information. Please use a different prompt."
                elif "Violence" in error_code:
                    if has_chinese:
                        user_msg = "输入文本可能包含激进行为相关信息，请您使用其他 prompt"
                    else:
                        user_msg = "Input text may contain violence information. Please use a different prompt."
                else:
                    if has_chinese:
                        user_msg = "输入文本可能包含敏感信息，请您使用其他 prompt"
                    else:
                        user_msg = "Input text may contain sensitive information. Please use a different prompt."
            ...
            if "InputTextSensitiveContentDetected" in error_code:
                if has_chinese:
                    ...
                    user_msg = "Input text may contain sensitive information. Please modify and retry."
            ...
            if "InputImageSensitiveContentDetected" in error_code:
                if has_chinese:
                    ...
                    user_msg = "Input image may contain sensitive information. Please modify and retry."
    ...
    if status_code == 401 or error_code == "Unauthorized":
        ...
        return LLMAccessDeniedError(
            f"Unauthorized: {error_message}", provider="doubao", error_code=error_code
        ), user_msg
    ...
    if status_code == 403 or error_code == "Forbidden":
        ...
        return LLMAccessDeniedError(f"Forbidden: {error_message}", provider="doubao", error_code=error_code), user_msg
    ...
    if status_code == 404 or error_code == "NotFound":
        ...
        if "InvalidEndpointOrModel" in error_code or "NotFound" in error_code:
            if "ModelNotOpen" in error_code or "not activated" in error_msg_lower:
                if has_chinese:
                    user_msg = "当前账号暂未开通模型服务，请前往火山方舟控制台开通管理页开通对应模型服务"
                else:
                    user_msg = "Model service not activated. Please activate the model service in Ark Console."
            ...
            if "ModelIDAccessDisabled" in error_code or (
                ...
                if has_chinese:
                    user_msg = "你的账号不允许使用模型ID来调用模型，请使用有权限的推理接入点 ID"
                else:
                    user_msg = "Model ID access disabled. Please use a custom endpoint ID instead."
                return LLMModelNotFoundError(
            ...
            if "UnsupportedModel" in error_code or "does not support" in error_msg_lower:
                if has_chinese:
                    ...
                    user_msg = (
                        "Model does not support this feature. Please refer to documentation for compatible models."
                    )
                return LLMModelNotFoundError(
            ...
            if has_chinese:
                ...
                user_msg = "Model or endpoint does not exist or you do not have access to it."
            return LLMModelNotFoundError(
        ...
        return LLMProviderError(f"Not found: {error_message}", provider="doubao", error_code=error_code), user_msg

    # ===== 429 TooManyRequests - Rate Limit Errors =====
    if (
        status_code == 429
        or error_code == "TooManyRequests"
        or "RateLimit" in error_code
        or "QuotaExceeded" in error_code
    ):
        ...
        if "EndpointRPMExceeded" in error_code or ("rpm" in error_msg_lower and "endpoint" in error_msg_lower):
            if has_chinese:
                user_msg = "请求所关联的推理接入点已超过 RPM (Requests Per Minute) 限制, 请稍后重试"
            else:
        ...
        if "EndpointTPMExceeded" in error_code or ("tpm" in error_msg_lower and "endpoint" in error_msg_lower):
            if has_chinese:
                user_msg = "请求所关联的推理接入点已超过 TPM (Tokens Per Minute) 限制, 请稍后重试"
            else:
        ...
        if "ModelAccountRpmRateLimitExceeded" in error_code or (
            ...
            if has_chinese:
                ...
                user_msg = "Model account RPM limit exceeded. Please try again later or contact platform support."
        ...
        if "ModelAccountTpmRateLimitExceeded" in error_code or (
            ...
            if has_chinese:
                user_msg = "请求已超过帐户模型 TPM (Tokens Per Minute) 限制: 请您稍后重试, 或者联系平台技术同学进行解决"
            else:
                user_msg = "Model account TPM limit exceeded. Please try again later or contact platform support."
        ...
        if "ModelAccountIpmRateLimitExceeded" in error_code or ("ipm" in error_code_lower):
            if has_chinese:
                user_msg = "请求已超过账户模型 IPM (Images Per Minute) 限制: 请您稍后重试, 或者联系平台技术同学进行解决"
            else:
                user_msg = "Model account IPM limit exceeded. Please try again later or contact platform support."
        ...
        if "QuotaExceeded" in error_code or "quota" in error_msg_lower:
            if "free trial" in error_msg_lower or "免费试用" in error_message:
                if has_chinese:
                    ...
                    user_msg = "Free trial quota exhausted. Please activate model service in Ark Console."
            ...
            if has_chinese:
                ...
                user_msg = "Quota exceeded. Please try again later or contact platform support."
            return LLMQuotaExhaustedError(
        ...
        if "ServerOverloaded" in error_code or "server overload" in error_msg_lower or "资源紧张" in error_message:
            if has_chinese:
                user_msg = "服务资源紧张，请您稍后重试。常出现在调用流量突增或刚开始调用长时间未使用的推理接入点"
            else:
                user_msg = (
        ...
        if "SetLimitExceeded" in error_code or "inference limit" in error_msg_lower or "推理限额" in error_message:
            if has_chinese:
                ...
                user_msg = (
                    "Inference limit reached. Please adjust limit in Ark Console or disable Safe Experience Mode."
                )
            return LLMQuotaExhaustedError(
        ...
        if "InflightBatchsizeExceeded" in error_code or (
            ...
            if has_chinese:
                user_msg = "您已经达到当前充值金额下的最大并发数限制，您可以充值解锁更大并发额度或降低并发数"
            else:
                user_msg = (
