...
Examples of relationship-only inputs (format "A to B" means left=A, right=B):
...
- "National mascot to Country" → Generate: Panda→China, Moose→Canada, Eagle→USA, Kangaroo→Australia, Lion→UK, Bear→Russia
...
2. 从多个维度对中心词进行描述
请输出一个包含以下字段的JSON对象：
topic: "{topic}"（如果需求中明确指定主题，必须完全匹配）
...
- title: "{topic}" (if specified in request, use EXACT title)
